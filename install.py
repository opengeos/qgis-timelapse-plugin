...
def get_qgis_plugins_path(profile: str = "default", custom_path: str = None) -> Path:
    ...
    if system == "Linux":
        ...
        paths = [
            ...
            home / ".var" / "app" / "org.qgis.qgis" / "data" / "QGIS" / "QGIS3" / "profiles" / profile / "python" / "plugins",
            # Snap installation
            home / "snap" / "qgis" / "current" / ".local" / "share" / "QGIS" / "QGIS3" / "profiles" / profile / "python" / "plugins",
        ]
    elif system == "Darwin":  # macOS
        paths = [
            home / "Library" / "Application Support" / "QGIS" / "QGIS3" / "profiles" / profile / "python" / "plugins",
        ]
    elif system == "Windows":
        appdata = os.environ.get("APPDATA", str(home / "AppData" / "Roaming"))
        paths = [
            Path(appdata) / "QGIS" / "QGIS3" / "profiles" / profile / "python" / "plugins",
        ]
    else:
...
Examples:
  ...
  python install.py --qgis-path /path  # Install to custom path
    ...
    parser.add_argument(
        "--no-deps",
        action="store_true",
        help="Skip dependency installation prompt"
    )
    parser.add_argument(
        "--qgis-path",
