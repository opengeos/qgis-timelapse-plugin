#!/usr/bin/env python3
"""
Package the QGIS Timelapse plugin for upload to the official QGIS plugin repository.

This script creates a zip file with the following characteristics:
- Root folder named 'timelapse' containing all plugin files
...
Examples:
    ...
    python package_plugin.py --output /path/to/output/timelapse.zip
"""
...
def package_plugin(
    ...
    Args:
        source_dir: Path to the timelapse plugin directory
        output_path: Optional path for the output zip file
        target_name: Name for the root folder in the zip (default: 'timelapse')
    ...
    if output_path is None:
        ...
        output_path = source_dir.parent / "dist" / zip_name
    ...
    with zipfile.ZipFile(zip_path, "r") as zipf:
        ...
        for required in required_files:
            if required in names:
                print(f"  ✓ Found required: {required}")
            else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Package QGIS Timelapse plugin for repository upload",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output path for the zip file (default: dist/timelapse-{version}.zip)",
    )
    parser.add_argument(
        "--source",
        "-s",
        type=Path,
        default=None,
        help="Source directory (default: timelapse/ in the same folder as this script)",
    )
    parser.add_argument(
        "--name",
        "-n",
